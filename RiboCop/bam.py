...
def count_rna_bam(bam, protocol, prefix):
    """Count RNA bam to test theta dist
       It is used by internal test only
    ...
    for r in tqdm(bam.fetch(until_eof=True)):
        ...
        if protocol == 'forward':
            ...
            strand = flipped[map_strand]
        for pos in ref_positions:
            # convert bam coordinate to one-based
            alignments[strand][(chrom, pos + 1)] += 1
